...
log_aircraft1_ev <- data.frame(
      ...
      cum_var_explained = cumsum(log_aircraft1_pr$sdev^2 / sum(log_aircraft1_pr$sdev^2))
    )
...
log_aircraft2_ev <- data.frame(
      ...
      cum_var_explained = cumsum(log_aircraft2_pr$sdev^2 / sum(log_aircraft2_pr$sdev^2))
    )
...
In the figures above, we can see that the first component of both plots are greater than 35. These principal components explain 99% of the variation in both periods. The figures show that the eigenvalues start to form a stairght line after the second principal component.
