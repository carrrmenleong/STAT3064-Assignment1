...
In the figures above, we can see that the first component of both plots are greater than 35. These principal components explain 99% of the variation in both periods. The figures show that the eigenvalues start to form a stairght line after the second principal component. The first eigenvalue increaased by approximately 2 times in the second period.
...
The above figure shows 6 score plots. The 3 plots at the top are the score plots for period 1 and the 3 plots at the bottom are for period 2. The range of values is larger in period 2 than in period 1 which also means that the observations are more spread out in period 2. We can see that observations are more concentrated on the point of origin in period 1 than period 2. The PC1/PC2 plots look like the letter T and the shape looks more distinctive in period 2.
...
```{r}
fviz_pca_var(log_aircraft1_pr, col.var="contrib",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             repel = TRUE # Avoid text overlapping
             ) + 
  labs(x = "PC1", y = "PC2") +
  ggtitle("Biplot - Period 1")
fviz_pca_var(log_aircraft2_pr, col.var="contrib",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             repel = TRUE # Avoid text overlapping
             ) + 
  labs(x = "PC1", y = "PC2") +
...
logWeight contributed the most in the first period whereas logPower contributed the most in the second period. The contribution from logPower increased drastically in period 2 and logRange has lesser contributions in period 2 than period 1. The contributions from the last 4 variables seem insignificant compared to the contributions from logWeight and logPower.

## (e) Based on your analysis, discuss the main changes that have occurred over time.

Weight and power are strongly positively correlated are likely to increase even more in the future. The correlation between speed and range decreases over time and they are now uncorrelated. Aircraft are more specialised for speed or range now.
